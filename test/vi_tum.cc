...
int main(int argc, char *argv[])
{
    if(argc < 4)
    {
        cerr << endl << "Usage: ./vi_tum  settings_Path sequence1_Path time1_Path (sequenceN_Path timeN_Path) file_Name" << endl;
        return 1;
    }
    ...
    if (bFileName)
    {
        const string fFile =  "./Tum/f_" + string(argv[argc-1]) + ".tum";
        const string kfFile =  "./Tum/kf_" + string(argv[argc-1]) + ".tum";
		const string PointCloudFile =  "./Tum/pointclouds_" + string(argv[argc-1]) + ".txt";
