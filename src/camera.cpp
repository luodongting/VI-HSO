              				 ...
              				 undist_map2_(height_, width_, CV_16SC2)
{
	...
	cv::initUndistortRectifyMap(cvK_, cvD_, cv::Mat_<double>::eye(3,3), cvK_, cv::Size(width_, height_), CV_16SC2, undist_map1_, undist_map2_);
...
void EquidistantCamera::distortPixelEquidistant(const Eigen::Vector2d& pixel_location, Eigen::Vector2d* distorted_pixel_location)
{
	float x = pixel_location[0];
    float y = pixel_location[1];
