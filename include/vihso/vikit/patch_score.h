...
public:
    ...
    static const int threshold_  = 2000*patch_area_;
    float* ref_patch_;
    float sumA_=0, sumAA_=0;

    ZMSSD_F(float* ref_patch) : ref_patch_(ref_patch)
    {
        float sumA_uint=0, sumAA_uint=0;
        for(int r = 0; r < patch_area_; r++)
        {
            float n = ref_patch_[r];
            sumA_uint += n;
            sumAA_uint += n*n;
        }
        sumA_ = sumA_uint;
    ...
    float computeScore(float* cur_patch) const
    {
        float sumB_uint = 0;
        float sumBB_uint = 0;
        float sumAB_uint = 0;
        for(int r = 0; r < patch_area_; r++)
        {
            const float cur_pixel = cur_patch[r];
            sumB_uint  += cur_pixel;
            sumBB_uint += cur_pixel*cur_pixel;
            sumAB_uint += cur_pixel * ref_patch_[r];
        }

        const float sumB = sumB_uint;
        const float sumBB = sumBB_uint;
        const float sumAB = sumAB_uint;
        return sumAA_ - 2*sumAB + sumBB - (sumA_*sumA_ - 2*sumA_*sumB + sumB*sumB)/patch_area_;
...
public:
    static const int patch_size_ = 2*HALF_PATCH_SIZE;
    static const int patch_area_ = patch_size_*patch_size_;
    float* hostPatch_ = NULL;
    float hostMean_ = 0;

    ZMNCC_F(float* ref_patch) : hostPatch_(ref_patch)
    {
        for(int r = 0; r < patch_area_; r++)
            hostMean_ += hostPatch_[r];

        hostMean_ /= patch_area_;
    }

    float computeScore(float* targetPatch) const
    {
        float targetMean = 0;
        for(int r = 0; r < patch_area_; r++)
            targetMean += targetPatch[r];

        targetMean /= patch_area_;

        float numerator = 0, demoniator1 = 0, demoniator2 = 0;
        for(int i = 0; i < patch_area_; i++)
        {
            float h = hostPatch_[i]-hostMean_;
            float t = targetPatch[i]-targetMean;
            numerator   += h*t;
            demoniator1 += h*h;
            demoniator2 += t*t;
        }

        return (numerator / (std::sqrt(demoniator1*demoniator2) + 1e-12));
    }
};
} 
} 
